...
class DetailTourismActivity : AppCompatActivity() {
    ...
    private fun showDetailTourism(detailTour: Tourism?) {
        detailTour?.let {
            supportActionBar?.title = detailTour.name
            binding.contentDetailTourism.tvDetailDescription.text = detailTour.description
            Glide.with(this@DetailTourismActivity)
                .load(detailTour.image)
                .into(binding.ivDetailImage)

            var statusFavorite = detailTour.isFavorite
            setStatusFavorite(statusFavorite)
            binding.fab.setOnClickListener {
                statusFavorite = !statusFavorite
                detailTourismViewModel.setFavoriteTourism(detailTour, statusFavorite)
                setStatusFavorite(statusFavorite)
            }
        }
    }
